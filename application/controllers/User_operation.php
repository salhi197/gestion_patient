...
class User_operation extends CI_Controller {
    ...
    public function setup()
    {
        ...
        */
        ...
        $this->form_validation->set_rules('footer_1', 'footer_1', 'required');
        $this->form_validation->set_rules('footer_2', 'footer_2', 'required');
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $error['specialite'] = form_error('specialite');
            $error['fax'] = form_error('fax');
            $error['footer_1'] = form_error('footer_1');
            $error['footer_2'] = form_error('footer_2');
        ...
        else
        {
            ...
            if ( ! $this->upload->do_upload('logo')) // file upload in folder
            {
                ...
                $dataedit = array(
                    ...
                    'doctor_name'=>$data['dname'],
                    'email'=>$data['email'],
                    'mobile'=>$data['phone'],
                    'password'=>base64_encode($data['password']),
                    'logo'=>$dataup['upload_data']['file_name'],
                    'favicon'=>$dataup1['upload_data']['file_name'],
                    'title'=>$data['title'],
                    'specialite' => $data['specialite'],
                    'fax' => $data['fax'],
                    'footer_1' => $data['footer_1'],
                    'footer_2' => $data['footer_2'],
                );
                 // array arrange based on table
